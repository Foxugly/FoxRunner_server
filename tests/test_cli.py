...
class CliTests(unittest.TestCase):
    def test_export_plan_writes_json_file(self):
        with TemporaryDirectory() as tmp:
            ...
            with (
                patch("app.main.build_runtime_services", return_value=fake_service),
                patch("app.main.load_config", return_value=object()),
                patch("sys.argv", ["main.py", "--export-plan", str(output_path)]),
            ):
    ...
    def test_history_accepts_execution_id_filter(self):
        ...
        with (
            patch("app.main.build_runtime_services", return_value=fake_service),
            patch("app.main.load_config", return_value=object()),
            patch("sys.argv", ["main.py", "--history", "--history-execution-id", "exec42"]),
