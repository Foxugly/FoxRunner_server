...
class IdempotencyTest(TestCase):
    ...
    def test_get_returns_none_when_no_header(self):
        request = _request({})
        self.assertIsNone(get_idempotent_response(request, user_id="u1", payload={"x": 1}))

    def test_store_and_replay(self):
        request = _request({"Idempotency-Key": "key-1"})
        payload = {"a": 1}
        store_idempotent_response(request, user_id="u1", payload=payload, response={"ok": True})
        replay = get_idempotent_response(request, user_id="u1", payload=payload)
    ...
    def test_replay_with_different_payload_returns_409(self):
        ...
        store_idempotent_response(request, user_id="u1", payload={"a": 1}, response={"ok": True})
        with self.assertRaises(HttpError) as ctx:
            get_idempotent_response(request, user_id="u1", payload={"a": 2})
        self.assertEqual(ctx.exception.status_code, 409)

    def test_concurrent_insert_silent_success_for_same_payload(self):
        request = _request({"Idempotency-Key": "key-3"})
        IdempotencyKey.objects.create(user_id="u1", key="key-3", request_fingerprint=_fingerprint({"a": 1}), response={"ok": True})
    ...
    def test_concurrent_insert_with_different_payload_returns_409(self):
        ...
        IdempotencyKey.objects.create(user_id="u1", key="key-4", request_fingerprint=_fingerprint({"a": 1}), response={"ok": True})
        with self.assertRaises(HttpError) as ctx:
            store_idempotent_response(request, user_id="u1", payload={"a": 2}, response={"ok": False})
        self.assertEqual(ctx.exception.status_code, 409)
